...
//          Activator.getDefault().getInjector().injectMembers(this); // Слишком рано?
                ...
                return new CellLabelProvider()
                {
                    @Override public void update(ViewerCell cell)
                    {
                        ...
                        if (ir.isConnecting(ib))
                        {
                            ...
                            cell.setForeground(ThemeAwareColors.effectiveSystemColor(d, SWT.COLOR_DARK_GRAY));
                        }
                        else
                        {
                            cell.setText("Подключить"); //$NON-NLS-1$
                            cell.setForeground(ThemeAwareColors.effectiveSystemColor(d, SWT.COLOR_DARK_BLUE));
                        }
    ...
    private static void renderSessionCell(ViewerCell cell, LocalDateTime start, String version)
    {
        ...
        if (start != null)
        {
            ...
            cell.setForeground(ThemeAwareColors.effectiveSystemColor(d, SWT.COLOR_DARK_GREEN));
        }
        else
        {
