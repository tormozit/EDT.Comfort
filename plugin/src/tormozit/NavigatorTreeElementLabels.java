 ...
 * (Реквизиты, Макеты, группы типов и т.п.) не участвуют в тексте поиска и не считаются
 * совпадением фильтра. Они остаются видимыми как контейнеры пути к объектам метаданных.
 * Подсветка фрагментов в надписи — только при иерархическом запросе (секция родителя).
 * Корень конфигурации и сами объекты — вне этого правила.
 */
public final class NavigatorTreeElementLabels
{
    ...
    /**
     * Служебная папка навигатора EDT. НЕ ТРОГАТЬ ГРУППЫ: не фильтровать как объект;
     * показывать, если в поддереве есть совпадения. Подсветка в надписи — только
     * при иерархическом запросе (секция родителя), не при одном слове фильтра.
     */
