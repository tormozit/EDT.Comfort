...
public final class EditorTabIconDiagHook implements IStartup
{
     ...
     */
    static void applyWorkbenchTabImage(IEditorPart editor, Image image)
    {
    ...
    private static void applyTitleImage(IEditorPart editor, Image image)
    {
        ...
        clearDisposedOverride(mpart);
        if (mpart != null)
            mpart.getTransientData().put(OVERRIDE_ICON_KEY, image);
        try
        {
    ...
    private static boolean restoreItem(CTabItem item)
    {
        ...
        Image live = liveImage(editor, mpartOfItem(item));
        if (!usableImage(live))
            return false;
