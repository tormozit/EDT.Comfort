 ...
 * <ul>
 * <li>двойной клик по строке узла причины ({@link IStacktraceError}) — то же действие, что команда
 * «Остановка по исключению» ({@link StacktraceStopOnExceptionHandler#triggerFor}). Только для узла
 * причины, не для строк кадров стека ({@code IStacktraceFrame}) — у тех двойной клик уже штатно
 * переходит к исходнику в модуле, вешать туда ещё и это окно поверх — мешать навигации по коду;</li>
...
public final class StacktracesViewInteractionHook implements IStartup
{
    ...
    private static void installDoubleClick(Display display)
    {
        ...
        display.addFilter(SWT.MouseDoubleClick, event ->
        {
            ...
            if (element != null)
                StacktraceStopOnExceptionHandler.triggerFor(element);
        });
    }
