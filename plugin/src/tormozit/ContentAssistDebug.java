...
public final class ContentAssistDebug
{
    ...
    // ---- Замер задержек автодополнения ---------------------------------------
    //
    // Временная перф-инструментовка «assist-perf» (приёмник .tmp/temp-logs/assist-perf.log
    // и сторожевой поток снятия стека UI) снята. Методы оставлены пустыми, чтобы не трогать
    // ~30 мест вызова в горячем коде автодополнения — ничего в temp-logs больше не пишется.

    /** No-op: временный замер задержек снят. */
    public static void perfLog(String location, long elapsedMs, int thresholdMs, String extraJson)
    {
    }

    /** No-op: раньше открывала замеряемую секцию под сторожевым потоком. */
    public static long perfStart(String location)
    {
        return 0L;
    }

    /** No-op: раньше закрывала замеряемую секцию и писала строку в лог. */
    public static void perfEnd(String location, long startNanos, String extraJson)
    {
    }

    /** No-op: раньше писала разовую отметку в лог. */
