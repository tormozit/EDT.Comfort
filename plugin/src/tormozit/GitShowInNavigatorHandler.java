...
public class GitShowInNavigatorHandler extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (part == null || !GitChangedFileMenuHook.isGitView(part))
            return null;
        Global.log("GitShowInNavigator: called part=" + part.getSite().getId()); //$NON-NLS-1$
        ...
        if (selection == null || selection.size() != 1) {
            Global.log("GitShowInNavigator: selection failed, size=" + (selection == null ? 0 : selection.size())); //$NON-NLS-1$
            return null;
        }

        Object element = selection.getFirstElement();
        Global.log("GitShowInNavigator: element class=" + element.getClass().getName() + " toString=" + element); //$NON-NLS-1$ //$NON-NLS-2$

        IFile file = GitChangedFileMenuHook.resolveFile(part, element);
        if (file == null || !file.exists()) {
            Global.log("GitShowInNavigator: file=null for " + element); //$NON-NLS-1$
            return null;
        }
        Global.log("GitShowInNavigator: file=" + file.getFullPath()); //$NON-NLS-1$

        EObject eObject = GitChangedFileMenuHook.resolveEObject(file);
        if (eObject == null) {
            Global.log("GitShowInNavigator: eObject=null for " + file.getFullPath()); //$NON-NLS-1$
            return null;
        }
