...
$script:ComfortRepoRoot = if ($PSScriptRoot) { Split-Path (Split-Path $PSScriptRoot -Parent) -Parent } else { 'C:\VC\EDT.Comfort' }
...
function Assert-ComfortOsgiVersionSync {
    ...
    $msg += @(
        ...
        'Fix: powershell -File site\scripts\restore-main.ps1 (close Eclipse first).',
        'Before Run: powershell -File site\scripts\assert-comfort-osgi.ps1'
    )
