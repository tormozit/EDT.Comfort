...
## Автодополнение BSL: первое заполнение словаря — только UI

**Первое** заполнение корневого списка слов в **экземпляре редактора** (`SmartContentAssistProcessor`) — только на UI-потоке, штатным `delegate.compute` / `showPossibleCompletions`. **Запрещено** уносить его в Job / `scheduleWordListInBackground` / `prepareWordListAutoOpen` (`wordDefer`).

Холодный фон возвращает 2–3 шаблона (`~Метка` …), это пишется в `fullListCache` и дальше фильтруется как «полный» словарь. Флаг: `wordListSeededOnUi`. Пока `false` — только UI. После успешного UI-compute в этом экземпляре повторные открытия того же редактора можно считать в фоне.

Не путать с другими исключениями: Ctrl+Space — UI (пользователь ждал список); список **членов после «.»** можно считать в фоне. Задержка ввода на большом модуле **не отменяет** это правило: первое заполнение всё равно UI.
