...
## Память об инциденте — не больше 10 КБ

Один файл памяти об одном инциденте (задаче, баге, разборе) — **не более 10 КБ**. Превысил — сокращать сразу, а не откладывать до общей чистки.

Что остаётся: итоговое состояние и статус подтверждения; механизм и место правки; проверенные тупики (чтобы не ходить туда снова); стоящие инструкции («не удалять диагностики», «не звать `create()` повторно»).

Что уходит: хроника попыток по времени, опровергнутые гипотезы во всех подробностях, списки удалённых полей и методов, пересказ уже отражённого в коде и коммитах.

Не дробить инцидент на несколько файлов ради обхода лимита: если материала на два файла — значит, это два разных факта, и они должны различаться по теме, а не по номеру части.

## Никогда не подавлять полосу прокрутки

Полоса прокрутки — признак обрезки содержимого. Убрать её (в том числе принудительным `layout`/`setSize`/`pack`, чтобы «влезло») значит спрятать обрезку, а не устранить её.
...
| Подменю «Комфорт» (та же команда внутри)   | —              | без                | без                |
| Внутри окна коллекции/скелета              | —              | без                | без                |
| Eclipse View, Preferences, тосты, окна EDT | без            | по контексту       | без                |


Примеры: `shell.setText(Global.withPluginWindowTitle("Коллекция …"))`; для кнопок/колонок/тулбаров вне окна — `setToolTipText("…" + Global.pluginSignForTooltip())`; внутри окна — `setToolTipText("Видимость и порядок колонок")`.

Суффикс говорит «это добавил плагин Комфорт». В подменю «Комфорт» это уже сказано названием подменю.

**Пункт контекстного меню — только `ComfortSubmenuHelper.setMenuItemTooltip(item, text)`**, без ручного `+ Global.pluginSignForTooltip()`. Это единственная точка решения про суффикс для команд меню: пункт сам знает своё место — внутри подменю «Комфорт» суффикс не ставится, в штатном меню (в т.ч. дубль той же команды в его корне) ставится. Текст переносится через `TooltipText.wrap` автоматически. Выпадающее меню кнопки «Комфорт» (не каскадный пункт) пометить `menu.setData(ComfortSubmenuHelper.SUBMENU_MARKER, Boolean.TRUE)` — тогда хелпер тоже сочтёт его подменю «Комфорт».

Кнопки, колонки, тулбары, `Action`, `ToolItem` — по-прежнему `setToolTipText(... + Global.pluginSignForTooltip())` вручную (хелпер только для `MenuItem`).
...
## Имя хука — по панели/окну, не по первой задаче

Хук панели, окна или диалога называть по тому, **что** он дорабатывает, а не по первой доработке: `FindReplaceDialogHook`, а не `FindReplaceLiveMatchCountHook`. Вторая доработка того же места идёт в тот же класс, и узкое имя сразу становится ложным — приходится переименовывать вместе со всеми ссылками (`plugin.xml`, javadoc).

Узкая задача в имени допустима, только если хук намеренно живёт отдельно от «общего» хука того же места (уже есть `XxxHook`, а этот — самостоятельная функция со своим жизненным циклом).
...
Только журнал «Комфорт» (`Global.log` / `GlobalLog`) при `Global.isLogEnabled()` (Параметры → Комфорт → «Вести журнал»). В модулях — `*Debug` с `TAG`, проверка флажка; эталон: `DebugInspectorDebug`.
...
`Global.tempLog(topic, text)` — стандартный приёмник временных логов: `Global.tempLog("тема", "текст")` — одна строка с меткой времени в файл `.tmp/temp-logs/<тема>.log`. Каждая тема — отдельный файл. Не зависит от флажка «Вести журнал» и не пишет в журнал «Комфорт» — независимый канал наравне с остальными приёмниками временных логов. Вся папка `.tmp/temp-logs/` очищается автоматически при каждом старте плагина.

## Проверки EDT — только в бандле `tormozit.comfort.checks`

Расширение `com.e1c.g5.v8.dt.check.checks` объявляется **только** в `plugin.checks/plugin.xml`. В `plugin/plugin.xml` проверок быть не должно.

Причина: EDT хранит в базе модели проекта версию бандла-поставщика проверок и при её изменении (`Version.equals`, квалификатор учитывается) зовёт `scheduleValidation` с пустым набором объектов — полную перепроверку проекта, десятки минут на большой конфигурации. Версия основного бандла меняется каждой сборкой, версия `plugin.checks` — фиксированная.

**Правя логику существующей проверки, поднять `Bundle-Version` в `plugin.checks/META-INF/MANIFEST.MF`** (`1.0.0` → `1.0.1`). Добавление/удаление проверки версии не требует — EDT видит это по составу id. Забыли — лечится командой «Проверить» на корневом узле проекта.

Категория проверок — одна (`tormozit.comfort.checks`, название «Комфорт»). Новые проверки класть в неё, а не заводить свою: две категории с одинаковым названием дают в дереве проверок EDT два одинаковых узла.

Идентификаторы проверок живут в основном бандле (`tormozit.ComfortCheckIds`), потому что на них ссылаются хуки, а обратная зависимость дала бы кольцо. Значения идентификаторов не менять: по ним хранятся настройки, скрытия и уже посчитанные маркеры. Подробности — `plugin.checks/README.md`.
